...
describe Movie do
  describe '.find_related_movies' do
    ...
    context 'director exists' do
      it 'finds similar movies correctly' do
        expect(Movie.related_movies(movie1.title)).to eql(['Catch me if you can', "Schindler's List"])
        expect(Movie.related_movies(movie1.title)).to_not include(['Seven'])
        expect(Movie.related_movies(movie2.title)).to eql(['Seven'])
      end
    end

    context 'director does not exist' do
      it 'handles sad path' do
        expect(Movie.related_movies(movie4.title)).to eql(nil)
      end
    end
  end
