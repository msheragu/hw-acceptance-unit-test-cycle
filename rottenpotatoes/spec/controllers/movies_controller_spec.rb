...
describe MoviesController do
  describe 'Search movies by the same director' do
    it 'should call Movie.similar_movies' do
      expect(Movie).to receive(:related_movies).with('Aladdin')
      get :director_search, { title: 'Aladdin' }
    end

    it 'should assign similar movies if director exists' do
      movies = ['Seven', 'The Social Network']
      Movie.stub(:related_movies).with('Seven').and_return(movies)
      get :director_search, { title: 'Seven' }
      expect(assigns(:related_movies)).to eql(movies)
    end

    it "should redirect to home page if director isn't known" do
      Movie.stub(:related_movies).with('No name').and_return(nil)
      get :director_search, { title: 'No name' }
